...
const AgentWallet = () => {
	...
	const { transactions, loading } = useUserTransactions(currentUser?.uid);

	let bottomsheet = (
		<WithdrawToBank
			isOpen={openWithdrawBottomsheet}
			onClose={() => setOpenWithdrawBottomSheet(false)}
		/>
	);
			...
			<div className="bg-primary w-full rounded-lg p-5 flex flex-col">
				...
				<div className="flex justify-between">
					<div className="space-y-3">
						<div className="flex gap-2 text-amber-100">Available</div>
						<p className={`${typography.paragraph} text-white text-xs`}>
							{userInfo?.wallet?.availableBalance ?? 0}
						</p>
					</div>
					<div className="space-y-3">
						<div className="flex gap-2 text-amber-100">Escrow Funds</div>
						<p className={`${typography.paragraph} text-white text-xs`}>
							{userInfo?.wallet?.escrowBalance ?? 0}
						</p>
					</div>
				</div>
			</div>
